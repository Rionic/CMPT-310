...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        visitedcorners = state[1]
                #add unvisitted corners
        if node in self.corners and (node not in visitedcorners):
            visitedcorners.append(node)
        return len(visitedcorners) == 4 #we found goal
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            #"*** YOUR CODE HERE ***"
              ...
              if not hitsWall:  
                nodeSuccessor = (nextx, nexty)
                successorVisited = list(visitedcorners)

                if nodeSuccessor in self.corners and (nodeSuccessor not in successorVisited):
                    successorVisited.append(nodeSuccessor)

                successor = ((nodeSuccessor, successorVisited), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        if corner not in visitedcorner:
            newcorner.append(corner)

    heuristic = 0

    while len(newcorner)>0:
        dist, corner = min([(util.manhattanDistance(currentstate,corner),corner) for corner in newcorner])
        
        currentstate = corner
        heuristic += dist
        newcorner.remove(corner)
