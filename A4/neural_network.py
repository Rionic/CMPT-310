...
class NeuralNet:
    ...
    def compute_gradients(self):
        """
        Computes the gradient of all weights with respect to the error. For each 
        layer L, stores the gradient in L._input_error_gradient, 
        where L._error_input_gradient is the gradient of loss with respect to the 
        input of layer L. Then passes that to the previous layer. 
        In other words, sets the previous layer's output gradient by the current 
        layer's input gradient.

        Hint 1: First compute the gradient of loss, then use that to initialize 
        backpropagation on the other layers.
        Hint 2: You can iterate through layers in reverse order using the for 
        loop: for layer in reversed(self._layers)

        """
        

        # YOUR CODE STARTS HERE (2)
        # prev layer output = current layer input
        self.loss.compute_gradient()
        val = self.loss.get_input_error_gradient()

        for layer in reversed(self._layers):
            layer._output_error_gradient = val
            layer.compute_gradient()
            val = layer._input_error_gradient
